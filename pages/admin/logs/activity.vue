<template>
  <section class="grid gap-4">
    <header>
      ...
      <h1 class="mt-1 text-3xl font-semibold text-[var(--pb-text)]">Activity logs</h1>
    </header>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-else-if="!rows.length">
            <td colspan="4" class="px-3 py-6 text-center text-[var(--pb-text-subtle)]">No activity logs for current filter.</td>
          </tr>
          <tr
            v-for="row in rows"
            :key="String(row.id)"

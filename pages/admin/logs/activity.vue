  ...
  <section class="grid gap-4">
    ...
    <div class="rounded-[var(--pb-radius-card-outer)] border border-[var(--pb-card-border)] bg-[var(--pb-card-bg)] p-4 shadow-[var(--pb-shadow-sm)]">
      <div class="grid gap-3 md:grid-cols-3 xl:grid-cols-4">
        ...
        <UInput v-model="filters.action" :placeholder="t('admin.logs.action')" />
        <UInput v-model="filters.resource_type" :placeholder="t('admin.logs.resourceType')" />
        <UInput v-model="filters.resource_id" :placeholder="t('admin.logs.resourceId')" />
        <UInput v-model="filters.search" :placeholder="t('admin.logs.searchActionDescription')" />
      ...
      <div class="overflow-auto">
        ...
        <thead class="sticky top-0 bg-[var(--pb-surface-subtle)] text-left text-[var(--pb-text-muted)]">
          <tr>
            ...
            <th class="px-3 py-2">{{ t('admin.logs.action') }}</th>
            <th class="px-3 py-2">{{ t('admin.logs.resource') }}</th>
            <th class="px-3 py-2">{{ t('admin.logs.logDescription') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="pending">
          ...
          <tr v-else-if="!rows.length">
            <td colspan="4" class="px-3 py-6 text-center text-[var(--pb-text-subtle)]">{{ t('admin.logs.emptyActivity') }}</td>
          </tr>
          <tr
            v-for="row in rows"
            :key="String(row.id)"

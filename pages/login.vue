<template>
  <main class="grid min-h-screen place-items-center bg-[var(--pb-app-bg)] px-5 py-12">
    <section class="w-full max-w-md rounded-[var(--pb-radius-card-outer)] border border-[var(--pb-card-border)] bg-[var(--pb-card-bg)] p-6 shadow-[var(--pb-shadow-md)]">
      <div class="mb-6">
        <p class="text-sm font-medium uppercase tracking-wider text-[var(--pb-link)]">Private access</p>
...
async function login() {
  ...
  try {
    ...
    errorMessage.value = error?.data?.message ?? error?.statusMessage ?? 'Invalid username or password'
  } finally {
    loading.value = false
  }
}
